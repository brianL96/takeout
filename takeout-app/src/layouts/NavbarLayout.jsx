...
const NavbarLayout = ({loginUsername, activateLogout, loginType, countClientCart, getClientCart, completeClientOrder, removeItemFromClientCart}) => {
  ...
  function getHiddenCart(){
    ...
    return [
      <div key={'key-client-navbarLayout'} className={`flex flex-row justify-end min-w-120 h-full ${hideCart}`}>
        <div className={`top-[25%] screen180:top-[10%] flex flex-col fixed h-3/4 screen180:h-9/10 w-1/2 min-w-120 overflow-y-auto bg-indigo-100 border border-black z-30 ${cartSlide}`}>
