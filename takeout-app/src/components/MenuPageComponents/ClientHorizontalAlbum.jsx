...
function getAmountOfSlots(){
  ...
  if(highlightArray.length === albumArray.length){
    ...
    while(index < length){
      ...
      arr.push(<ClientAlbumItem key={`AlbumItem-${albumArray[index].title}`} imageSRC={albumArray[index].image} index={index} highlight={borderHighlight} changeHighlight={changeHighlight} />);
      index++;
    }
    
    return [
      <div key={'key-GalleryContainer'} className='flex flex-col justify-center h-auto w-full min-w-120 mt-10 mb-10'>
