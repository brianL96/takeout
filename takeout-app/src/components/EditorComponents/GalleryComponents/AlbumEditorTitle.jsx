...
const AlbumEditorTitle = ({getAlbumItems, editAlbum, turnOffAlbumDeployment}) => {
    ...
    function getDeployedStatusMessage(){
        if(cardDepolyed === 'null'){
            ...
            return [<h1 className='ml-2 text-green-700'>Your photo gallery is deployed and visible to other users.</h1>];
        }
        else if(cardDepolyed === 'false'){
            return [<h1 className='ml-2 text-red-700'>Your photo gallery is not deployed. Your business is not displaying any images.</h1>];
        }
        else{
            return [];
        }
  ...
  return (
    <>
        <div className='flex flex-row justify-center w-full h-36 mt-20'>
