...
const ImageSelector = ({closeImageSelector, hideImageSelector, getAlbumItems, blocksArray, setCurrentSelectedImageTitle, defaultImageHighlight, editSelectedImageTitle, setEditSelectedImageTitle}) => {
  ...
  function insertAlbumItems(){
        ...
        if(albumItems === null || albumItems.deployed === false || albumItems.album.length === 0){
          arr.push(
            <div key={'key-ImageSelector-EmptyContainer'} className='flex flex-row justify-center h-40 w-full mt-1'>
        ...
        while(i < size){
          ...
          while(inRow < limit && i < size){
            ...
            arr2.push(
              <ImageSelectorItem key={`key-ImageSelectorItem-${albumArray[i].title}`} squareImage={albumArray[i].image} squareTitle={albumArray[i].title} squareBorder={squareBorder} squareIndex={i} changeSelectedIndex={changeSelectedIndex}/>
            );
            inRow++;
            i++;
          }
          if(inRow > 0){
            arr.push(
              <div key={`key-ImageSelectorItemsRow-${i}`} className='flex flex-row justify-start mt-1 mb-1 w-full min-h-40 h-auto'>
                {arr2}
              </div>
            );
          }
