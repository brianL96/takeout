...
const Navbar = ({loginUsername, activateLogout, loginType, countClientCart, changeHideCart}) => {
    ...
    function getCartNumber(count){
        ...
        let arr = [];
        if(count > 0 && count <= 9){
            arr.push(<div key={'key-cartCount-0to9'} className='flex flex-row justify-center mt-0 pt-0 ml-0.5 h-5 w-8 rounded-full text-sm text-white bg-amber-600'>{`${count}`}</div>);
        }
        else if(count > 9){
            arr.push(<div key={'key-cartCount-10up'} className='flex flex-row justify-center mt-0 pt-0 ml-0.5 h-5 w-8 rounded-full text-sm text-white bg-amber-600'>9+</div>);
        }
