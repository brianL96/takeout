...
const Cart = ({clientCart, completeClientOrder, removeItemFromClientCart, clientOrderTo}) => {
    ...
    function loadList(){
        ...
        if(clientCart === null || clientCart.length < 1){
            return [
                <div key={'key-empty-cart'} className='flex flex-row justify-center w-full mb-10'>
        ...
        while(i < size){
            arr.push(
                <div key={`key-cart-item-${i}`} className='bg-white border-b border-black flex flex-row justify-start w-full h-auto min-h-20 text-lg'>
        ...
        return [
            <div key={'key-complete-cart'} className='flex flex-col justify-start w-105 mb-10 border border-black'>
                <div className='flex flex-row justify-center h-16 w-full text-xl font-medium border-b border-black bg-gray-200 overflow-y-auto'>
                    <div className={`min-h-1/2 h-auto w-auto max-w-72 break-words text-wrap p-1 pt-3`}>
                        <h1>{clientOrderTo}</h1>
                    </div>
                </div>
    ...
    function getSummary(totalPrice){
        ...
        if(clientCart.length > 0){
            ...
            arr.push(
                <div key={'key-totalPriceContainer'} className='flex flex-col justify-start w-full h-auto bg-gray-200'>
            ...
            arr.push(
                <div key={'key-empty-cart-totalPrice'} className='flex flex-row justify-center items-center w-full h-20'>
                    <h2>Your Cart Is Empty</h2>
                </div>
            );
        }
