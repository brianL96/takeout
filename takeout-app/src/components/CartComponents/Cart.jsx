...
const Cart = ({clientCart, completeClientOrder, removeItemFromClientCart, clientOrderTo}) => {
    ...
    function loadList(){
        ...
        while(i < size){
            arr.push(
                <div className='bg-white border-b border-black flex flex-row justify-start w-full h-20 text-lg' key={`item${i}`}>
                    <div className='flex flex-row justify-center items-center w-3/5 h-full text-sm'>
                        {`${clientCart[i].itemName}`}
                    </div>      
                    <div className='flex flex-row justify-center items-center w-1/5 h-full text-sm'>
                        {`$${clientCart[i].itemPrice}`}
                    </div>
                    <div className='flex flex-row justify-center items-center w-1/5 h-full'>
